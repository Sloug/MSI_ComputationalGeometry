...
//
//-------------------------------------------------
Point::Point(const Point  & p) : CoordTuple(p[0],p[1],p[2]) {}    // cast constructor
...
{
	return Point(c[0]+vec.c[0],c[1]+vec.c[1],c[2]+vec.c[2]);
}
...
{
	return Vector(c[0]-pnt.c[0],c[1]-pnt.c[1],c[2]-pnt.c[2]);
}
...
double Vector::cosTheta(const Vector &v) const
{
    return (*this * v) / (this->getNorm() * v.getNorm());
//    return (v * *this) / (v.getNorm() * this->getNorm());
}
...
{
	return Vector(c[0]+vec.c[0],c[1]+vec.c[1],c[2]+vec.c[2]);
}
...
{
	return Vector(c[0]*s,c[1]*s,c[2]*s);
}
