...
public:
	CoordTuple(T x=T(0), T y=T(0), T w=T(0));
...
public:
	...
	Point(double x=0.0, double y=0.0, double w=1.0) : CoordTuple( x, y, w) {}
...
class Vector : public CoordTuple4d
{
	...
	friend class Line;
public:
	Vector(double x=0.0, double y=0.0, double w=0.0) : CoordTuple(x, y, w) {};
	Vector(const double        v[4]) : CoordTuple(v[0],v[1],v[2]) {};
	Vector(const CoordTuple4d& v   ) : CoordTuple(v[0],v[1],v[2]) {};
	Vector(const Vector&       v   ) : CoordTuple(v[0],v[1],v[2]) {};      // copy constructor
    ...
    double  cosTheta     (const Vector &v) const;
	...
	double  operator ^  (const Vector &v) const;		// result  =  (*this) x v // Attention: "^"-opertor has lower priority than e.g. "+"-operator, i.e. use brackets!!!
};
...
template <class T, unsigned D> CoordTuple<T,D>::CoordTuple(T x, T y, T w)
{
	c[0] = x;
	c[1] = y;
	if (D > 2) {
